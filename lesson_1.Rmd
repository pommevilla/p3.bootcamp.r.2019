...
We could then repeat this for each of the number of cylinders to get what we need. Howver, this is not scalable, and it relies on you knowing all the different levels of the variable you're looking at. This is obviously not feasible for larger datasets. To remedy this, we can use the `group_by` and `summarise` commands from `tidyverse`. You pass the variable you would like group the original data by to the `group_by` function, and from there, we can use the summarise function to get some summary statistics by passing different formulas to the function call. For instance, to get the mean mpg by cylinder count, you can do:
